...
- Python 3.11+ (проект создавался под современный Python)
- pip
- Рекомендуется использовать виртуальное окружение (`venv`).

Основная зависимость указывается в `requirements.txt`:
...
- Добавить автотесты для `calculator_view`.
- Вынести стили в отдельный CSS-файл и подключить статику Django.
- Добавить переключатель темы (светлая / тёмная).
- Добавить кнопку очистки истории вычислений.
